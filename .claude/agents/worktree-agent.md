...
2. **Ground in the plan** — re-read the relevant `Planning\NN_*_Plan.md` in full: Architecture Approach, Database Entities, APIs, UI/Screens, File Structure, Security Considerations, Test Strategy, Acceptance Criteria. This is what you build against. Per CLAUDE.md, client styling is **Tailwind CSS**: style new/updated components with Tailwind utility classes directly in the template rather than writing component `.scss`; only add `.scss` for something Tailwind can't express.
