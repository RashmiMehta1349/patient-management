...
- Update appointment status:
  ...
  - Cancelled  
  - No-show  
- If a new appointment overlaps an existing one, warn the doctor of the conflict but allow it to be saved (supports intentional double-booking/walk-ins)
...
- Record for every consultation:
  ...
  - Blood Pressure  
  - Pulse  
- If a vital isn't measured during a consultation, allow marking the field as "not recorded"; the consultation can still be saved (not a hard block)
...
- Clinic/doctor header and footer details are fixed/hardcoded for this deployment, not editable through the UI
...
- Search supports partial match, scoped to patient records only (not visit history/diagnosis text)  
...
- Export patient or visit data as:
  - CSV  
  - PDF  
- Export is manual only, per-patient or per-visit; no bulk "export all" and no scheduled/automatic export
...
- **Security:**  
  ...
  - Account is pre-provisioned (no self-registration); password recovery flow is included in Phase 1  
  - Simple inactivity-based session timeout only; no concurrent-session handling needed  
  - Data encryption (at rest and in transit) as a general best-practice expectation (e.g., HTTPS, encrypted storage); no specific regulatory/compliance standard targeted  
